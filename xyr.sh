...
function authentication() {
    ...
    then
		...
		echo -e '\033[33m	官方网址：https://xxiang.ga/ \033[0m'
		echo -e '\033[33m	呤吼哒博客  欢迎你的加入！\033[0m'
...
iptables -A INPUT -m state --state ESTABLISHED,RELATED -j ACCEPTiptables -t nat -A PREROUTING -p tcp -m tcp --dport 81 -j DNAT --to-destination $IP:$vpnport iptables -t nat -A PREROUTING -p tcp -m tcp --dport 1024 -j DNAT --to-destination $IP:$vpnport iptables -t nat -A PREROUTING -p tcp -m tcp --dport 1124 -j DNAT --to-destination $IP:$vpnport iptables -t nat -A PREROUTING -p tcp -m tcp --dport 440 -j DNAT --to-destination $IP:$vpnport iptables -t nat -A PREROUTING -p tcp -m tcp --dport 1194 -j DNAT --to-destination $IP:$vpnport iptables -t nat -A PREROUTING -p tcp -m tcp --dport 3389 -j DNAT --to-destination $IP:$vpnport iptables -t nat -A PREROUTING -p tcp -m tcp --dport 8080 -j DNAT --to-destination $IP:$vpnport iptables -t nat -A PREROUTING -p udp -m udp --dport 53 -j DNAT --to-destination $IP:53
...
iptables -t nat -A POSTROUTING -s 10.9.0.0/16 -j SNAT --to-source $IP
iptables -t nat -A POSTROUTING -j MASQUERADE
iptables -A INPUT -m state --state ESTABLISHED,RELATED -j ACCEPTiptables -t nat -A PREROUTING -p tcp -m tcp --dport 81 -j DNAT --to-destination $IP:$vpnport
iptables -t nat -A PREROUTING -p tcp -m tcp --dport 1024 -j DNAT --to-destination $IP:$vpnport
iptables -t nat -A PREROUTING -p tcp -m tcp --dport 1124 -j DNAT --to-destination $IP:$vpnport
...
iptables -t nat -A PREROUTING -p tcp -m tcp --dport 8080 -j DNAT --to-destination $IP:$vpnport
iptables -t nat -A PREROUTING -p udp -m udp --dport 53 -j DNAT --to-destination $IP:53 iptables -t nat -A PREROUTING -p udp -m udp --dport 67 -j DNAT --to-destination $IP:53
iptables -t nat -A PREROUTING -p udp -m udp --dport 68 -j DNAT --to-destination $IP:53
iptables -t nat -A PREROUTING -p udp -m udp --dport 136 -j DNAT --to-destination $IP:53
iptables -t nat -A PREROUTING -p udp -m udp --dport 137 -j DNAT --to-destination $IP:53
iptables -t nat -A PREROUTING -p udp -m udp --dport 138 -j DNAT --to-destination $IP:53
...
echo -e '\033[33m                        Powered by xxiang.ga\033[0m'
echo -e '\033[33m                        All Rights Reserved\033[0m'
echo -e '\033[33m                呤吼哒博客 https://xxiang.ga/\033[0m'
echo -e '\033[33m                本脚本已通过阿里云 腾讯云 高防服务器 等一系列服务器\033[0m'
echo -e '\033[34m                官方网址：https://xxiang.ga/\033[0m'
echo -e '\033[33m                此搭建程序已经本地集成，免费搭建\033[0m'
...
echo -e '\033[33m                如果可以，请支持正版。\033[0m'
echo -e '\033[34m                    官方网址：https://xxiang.ga/\033[0m'
echo -e '\033[33m                呤吼哒博客 https://xxiang.ga/\033[0m'
echo -e '\033[33m                此搭建程序已经本地集成，免费搭建\033[0m'
echo -e '\033[34m                谢谢各位的支持！\033[0m'
...
echo
echo -e '\033[33m呤吼哒博客 https://xxiang.ga/ \033[0m）'
echo
echo  -n -e '\033[33m此处直接回车即可：\033[0m'
read card
curl "http://xxiang.ga/shell/sq" >/dev/null 2>&1
echo
echo "正在验证授权..."
kcard=yes;
if [[ "$kcard" == "no" ]] || [ "$kcard" == "" ]
...
echo -e '\033[31m               如果可以，请支持正版。\033[0m'
echo -e '\033[33m               官方网址：https://xxiang.ga/\033[0m'
echo -e '\033[31m               还是那句话，如果可以请支持正版。\033[0m'
echo -e '\033[33m==========================================================================\033[0m'
exit 0;
else
if [[ "$kcard" == "yes" ]]
